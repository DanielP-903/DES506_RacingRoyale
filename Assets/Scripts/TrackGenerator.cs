...
public class TrackGenerator : MonoBehaviour
{
    ...
    private void GenerateTrack()
    {
        for (int i = 0; i < noOfPieces - 1; i++)
        {
            ...
            float XdistBetweenEndAndStart = Mathf.Abs(generatedTrack[i + 1].startLocation.transform.position.x - generatedTrack[i].endLocation.transform.position.x);
            float ZdistBetweenEndAndStart = Mathf.Abs(generatedTrack[i + 1].startLocation.transform.position.z - generatedTrack[i].endLocation.transform.position.z);
