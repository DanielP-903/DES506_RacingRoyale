...
public class TrackGenerator : MonoBehaviour
{
    ...
    {
        if (noOfPieces <= _generatedTrack.Count)
        {
            for (int i = 0; i < noOfPieces - 1; i++)
            {
                directionEnd =
                    transform.worldToLocalMatrix.MultiplyVector(_generatedTrack[i].endLocation.transform.forward);
                directionStart =
                    transform.worldToLocalMatrix.MultiplyVector(_generatedTrack[i + 1].startLocation.transform.forward);
    ...
    private void GenerateTrack()
    {
        for (int i = 0; i < noOfPieces - 1; i++)
        {
            ...
            // Get Vector3.SIGNEDAngle between two LOCAL forward vectors at previous end and current start
            Vector3 forwardPrevEnd = transform.worldToLocalMatrix.MultiplyVector(_generatedTrack[i].endLocation.transform.forward);
            ...
            float angle = Vector3.SignedAngle(forwardCurrentStart, forwardPrevEnd, Vector3.up);
